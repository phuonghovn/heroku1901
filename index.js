...
app.get('/remove/:id', (req, res) => {
    ...
    .then(singer => {
        if (singer.image !== 'defaut.png') fs.unlinkSync(__dirname + '/public/' + singer.image);
         res.redirect('/');
    })
    .catch(error => res.send(error));
});
...
app.post('/add', upload.single('image'), (req, res) => {
    ...
    const image = req.file ? req.file.filename: 'defaut.png';
...
app.post('/update/:id', upload.single('image'), (req, res) => {
    ...
    .then(singer => {
        if (singer.image !== 'defaut.png' && req.file) {
            try {
                fs.unlinkSync(__dirname + '/public/' + singer.image);
            } catch (error) {
                console.log("Can't find " + singer.image + "file.")
            }
        }
        res.redirect('/');
    })
